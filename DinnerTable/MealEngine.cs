...
public sealed class MealEngine(Store db) {
 ...
 public void Apply(PlanState s,ActionRequest a){
  ...
  if(a.Type=="generate"){
   ...
   s.Meals=beam.OrderBy(p=>Cost(s,p)>s.Budget?1:0).ThenBy(p=>Cost(s,p)-p.Count(s.Favorites.Contains)*0.35m).First();s.Checked.Clear();
   ...
   if(a.Type=="remove")s.Meals.RemoveAt(a.Index.Value);
   ...
   s.Checked.Clear();
  }else if(a.Type is "favorite" or "dislike"){
